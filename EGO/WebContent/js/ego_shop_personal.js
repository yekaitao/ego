 ...
 $('#ego_personal').click(function () {
  ...
  $('#ego_address').click(function () {
    window.location.href = 'ego_shop_address.html'
	 ...
	 $.ajax({
		 type: 'POST',
		 url: 'http://localhost:8080/EGO/show_personal',
		 ...
		 success: function (data, msg, xhr) {
			 ...
			 if (data.code == "200") {
				 let nickname = data.allData.nickname
				 let upassword = data.allData.upassword
				 let phone = data.allData.phone
				 let email = data.allData.email
				 let uname = data.allData.uname
				 let gender = data.allData.gender
				 if (gender == 1) {
  ...
  $('#edit_save').click(function () {
    ...
    $.ajax({
      method: 'POST',
      url: 'http://localhost:8080/EGO/save_personal',
      data:JSON.stringify({uid:uid,upassword:upassword,uname:uname,gender:gender}),
      dataType: 'json',
      contentType:"application/json; charset=utf-8",
      success: function (data, msg, xhr) {
