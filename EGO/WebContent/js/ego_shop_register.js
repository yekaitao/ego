    ...
    $('#nickname').blur(function () {
      ...
      $.ajax({
        type: 'POST',
        url: 'http://localhost:8080/EGO/nickname',
    ...
    $('#phone').blur(function () {
      ...
      $.ajax({
        type: 'POST',
        url: 'http://localhost:8080/EGO/phone',
    ...
    $('#email').blur(function () {
      ...
      $.ajax({
        method: 'POST',
        url: 'http://localhost:8080/EGO/email',
    ...
    $('#yzmImg').click(function () {
      $('#yzmImg').attr('src', 'http://localhost:8080/EGO/validateCode?' + Math.random())
    })
    ...
    $('#register').click(function () {
      ...
      if (nicknameReg.test(nickname) && upasswordReg.test(upassword) && phoneReg.test(phone) && emailReg.test(
        ...
        $.ajax({
          method: 'POST',
          url: 'http://localhost:8080/EGO/register',
          data:JSON.stringify({nickname:nickname,upassword:upassword,phone:phone,email:email,myyzm:myyzm}),
          dataType: 'json',
   		  contentType:"application/json; charset=utf-8",
          success: function (data, msg, xhr) {
            console.log('异步请求注册API成功', data)
